...
void ClapTrap::beRepaired(unsigned int amount)
{
    if(_hitPoints > 0 && _energyPoints > 0)
    {
        _energyPoints--;
        _hitPoints += amount;
        std::cout << "ClapTrap " << _name << " is repaired for " << amount << " points! (Remaining Energy: " << _energyPoints << ")" << std::endl;
    }else {
        std::cout << "ClapTrap " << _name << " has no energy left or is already destroyed and cannot repair itself!" << std::endl;
    }
}
